...
class VersionAdmin(admin.ModelAdmin):
    ...
    def _model_key(self, obj, model=None):
        """
        Get key for save changes of model
        """
        if not obj and not model:
            raise Exception("Model or object must be set")
        if not obj:
            obj = ""
        instance_id = obj.id if hasattr(obj, "id") else obj
    ...
    def construct_change_message(self, request, form, formsets):
        """
        Override parent method for save updated fields.
        """
        changes = {}
