...
var xrayAd = {div:null,viewport:null,thresold:200,elements:[],adBlockCount:0,w:160,h:200,init:function() {
    this.div = $('#xrayAd');
    if (!this.div) {
        this.div = $('<div>', {id:'xrayAd',css:{position:'fixed',top:10,left:10,width:this.w,height:this.h,zIndex:10000,background:'rgba(0,0,0, 0.5)'}});
        this.div.appendTo($('body'));
    }
},viewportUpdate:function() {
    if (!this.viewport) {
        this.viewport = $('<div>', {id:'xrayAdViewport',css:{position:'absolute',width:this.w,height:10,zIndex:10001,background:'rgba(255,255,255, 0.3)'}});
        this.viewport.appendTo(this.div);
    }
    if (!this.viewThresoldTop) {
        this.viewThresoldTop = $('<div>', {id:'xrayAdThresold',css:{position:'absolute',width:this.w,height:1,zIndex:10002,background:'rgba(255,0,0, 0.5)'}});
    ...
    var vH = ($(window).height() / this.bodyHeight) * xrayAd.h,vT = ($(window).scrollTop() / this.bodyHeight) * xrayAd.h;
    this.viewport.css({height:vH,top:vT});
    this.viewThresoldTop.css({top:(($(window).scrollTop() - xrayAd.thresold) / this.bodyHeight) * xrayAd.h});
    this.viewThresoldBottom.css({top:(($(window).scrollTop() + xrayAd.thresold) / this.bodyHeight) * xrayAd.h + vH - 1});
        ...
        $.each(blocks, function(key, val) {
            ...
            if (xrayBlock.length && adBlock.length) {
                ...
                if (size.off) {
                    ...
                    xrayBlock.css({top:size.top,left:size.left,width:size.w,height:size.h,borderColor:bgColor});
                }
            }
        });
    }
    ...
    this.init();
    var adBlock = $('<div>', {'class':'xrayAdBlock','css':{position:'absolute',background:'#ffffff',border:'1px solid #FF0071',top:0,left:0,width:0,height:0,zIndex:10003}});
    $.each(el, function() {
...
(function($) {
    ...
    $.fn.lazyLoadAd = function(options) {
        var settings = {threshold:0,failurelimit:1,forceLoad:false,event:"scroll",viewport:window,placeholder:false,onLoad:false,onComplete:false,timeout:1500,debug:false,xray:false};
        if (options) {
        ...
        this.each(function(_index, _value) {
            ...
            self.bind("debug", function(e, status) {
                status = status || 'start';
                if (settings.xray) {
                    if (status == 'start')self.trigger('onLoadXray'); else if (status == 'error')self.trigger('onErrorXray'); else if (status == 'complete')self.trigger('onCompleteXray');
                }
                if (settings.debug) {
                    if (status == 'start')self.css({border:'3px solid orange'}); else if (status == 'error')self.css({border:'3px solid red'}); else if (status == 'complete')self.css({border:'3px solid green'});
                }
            });
            ...
            self.bind('docWrite_overload', function() {
                ...
                document.write = document.writeln = function() {
                    ...
                    var html = '';
                    for (var i = 0; i < args.length; i++)html += args[i];
                    var testHTML = '',directWrite = false;
                    try {
                        testHTML = $(html);
            ...
            self.bind('evalCode', function() {
                var scripts = [],script,regexp = /<code[^>]*>([\s\S]*?)<\/code>/gi;
